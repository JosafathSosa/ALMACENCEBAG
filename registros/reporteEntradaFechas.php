...
$pdf->SetTitle('Informe de Entradas');
...
$pdf->Cell(100, 6, 'LISTA DE ENTRADAS', 0, 0, 'C');
...
$sql = "SELECT `entrada`.`id_entrada`,`entrada`.`producto`, `entrada`.`recibe`,`entrada`.`cantidad`,`entrada`.`entrega`,`entrada`.`fecha`, `proveedor`.`nombre` FROM `entrada` LEFT JOIN `proveedor` ON `entrada`.`id_proveedor` = `proveedor`.`id_proveedor` WHERE `entrada`.`fecha` BETWEEN '$fechaInicio' and '$fechaFin'";
$resultado = mysqli_query($mysqli, $sql);

while ($dataRow = mysqli_fetch_array($resultado)) {

    $pdf->Cell(40, 6, $dataRow['nombre'], 1, 0, 'C');
    $pdf->Cell(35, 6,  $dataRow['producto'], 1, 0, 'C');
    $pdf->Cell(35, 6,  $dataRow['recibe'], 1, 0, 'C');
    $pdf->Cell(40, 6,  $dataRow['entrega'], 1, 0, 'C');
...
$pdf->Output('Reporte_Entrada_' . date('d_m_y') . '.pdf', 'I'); 
